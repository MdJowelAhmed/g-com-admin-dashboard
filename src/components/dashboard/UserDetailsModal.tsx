...
import { resolveUserMediaUrl } from '../../redux/api/userApi'
...
export default function UserDetailsModal({ user, open, onClose }: Props) {
  const profileImage = user?.profileImage
    ? resolveUserMediaUrl(user.profileImage)
    : ''
      ...
      {user && (
        <div className="space-y-5">
          <div className="flex items-center gap-4">
            ...
            <div className={`text-sm font-semibold ${statusStyles[user.status]}`}>
              {user.status}
            </div>
          </div>
          ...
          <dl className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <dd className="mt-1 text-sm font-medium text-white whitespace-pre-wrap">
                {user.about}
              </dd>
            </div>
          </dl>
...
function DetailRow({ label, value }: DetailRowProps) {
  return (
    <div>
      ...
      <dd className="mt-1 text-sm font-medium text-white capitalize">{value}</dd>
    </div>
  )
}
